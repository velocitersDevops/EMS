...
//    	Integer addressId = (Integer)session.getAttribute(EmployeeInterface.ADDRESSID);
		...
		// =====================================Merged Code Of Personal Information and
		// Address=======================
		...
		// taking address information data from request object and storing into address
		// object
		...
		// here appending addressLine2 data with AddressLine1(Addresline1 + Addresline1)
		// and storing into addressObject
		if (addressLine2Data != "") {
		...
		try {
			...
			// by using operationObject reference call updateEmployeeDetail method of
			// operation class
			...
			if (familyId == 0) // If family id of employee is 0 in employee table it means that employee has
								// not added his family details
			{
				// Calling method addFamilyDetails of Operations class using operationObject
				// reference
			...
			if (personalInfoid > 0 && addressId > 0) {
				...
				if (personalInfoCount > 0 && addressCount > 0) {
					// out.println("<h4 style='color: green;'> Records Upadate SuccessFully
					// !</h4>");
					RequestDispatcher requestDispaterObject = req
							.getRequestDispatcher("UpdateStatus.jsp?status=" + 1 + "");
					requestDispaterObject.forward(req, resp);
				} else if (personalInfoCount == 0 || addressCount > 0) {
					out.println(
							"<h4 style='color: green;'>Address SuccessFully </h4> <h4 style='color: red;'>Personal Info Faild </h4>");
					RequestDispatcher requestDispaterObject = req.getRequestDispatcher("UpdateStatus.jsp");
					requestDispaterObject.include(req, resp);
				} else if (personalInfoCount > 0 || addressCount == 0) {
					out.println(
							"<h4 style='color: green;'>Personal Info SuccessFully </h4> <h4 style='color: red;'>Address  Faild </h4>");
					RequestDispatcher requestDispaterObject = req.getRequestDispatcher("UpdateStatus.jsp");
					requestDispaterObject.include(req, resp);
				} else {
					// out.println("<h4 style='color: red;'> Records Not Upadate SuccessFully
					// !</h4>");
					RequestDispatcher requestDispaterObject = req
							.getRequestDispatcher("UpdateStatus.jsp?status=" + 0 + "");
					requestDispaterObject.include(req, resp);
				}
			} else {

				if (personalInfoid == 0 && addressId == 0) // If personalInfoid of employee is 0 in Personal_information
															// table it means that employee has not added his personal
															// detail
				{
					// we taking employee id from session now tamprary we taken empid is
					// personalinfoid
					...
					int personalinfoIdUpdateStatus = operationObject
							.updatePersonalInfoidInEmployeeTable(personalInfoid);
					if (personalInfoCount > 0 && addressCount > 0) {

						RequestDispatcher requestDispaterObject = req
								.getRequestDispatcher("UpdateStatus.jsp?status=" + 2 + "");
						requestDispaterObject.include(req, resp);
					} else {

						RequestDispatcher requestDispaterObject = req
								.getRequestDispatcher("UpdateStatus.jsp?status=" + 3 + "");
						requestDispaterObject.include(req, resp);
					}
